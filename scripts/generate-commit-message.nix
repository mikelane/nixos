...
4. Write your commit message in the imperative, _e.g._, 'Fix bug' and
...
5. Any further paragraphs come after blank lines.

Note:
- Bullet points are okay for the message body, too.
...
top of commit messages. It provides an easy set of rules for creating 
an explicit commit history; which makes it easier to write automated 
tools on top of. This convention dovetails with SemVer, by describing 
...
- types other than 'fix:' and 'feat:' are allowed, for example 
  ...
  recommends 'build:', 'chore:', 'ci:', 'docs:', 'style:', 
...
- Additional types are not mandated by the Conventional Commits 
  ...
  ''\(unless they include a 'BREAKING CHANGE'''\). A scope may be provided
  to a commit’s type, to provide additional contextual information and 
  is contained within parenthesis, e.g., 'feat''\(parser''\): add ability to
  parse arrays'.
...
4. A scope MAY be provided after a type. A scope MUST consist of a 
   noun describing a section of the codebase surrounded by parenthesis, 
   e.g., fix''\(parser''\):
...
8. One or more footers MAY be provided one blank line after the body. 
   ...
   :<space> or <space># separator, followed by a string value ''\(this is
   inspired by the git trailer convention''\).
9. A footer’s token MUST use - in place of whitespace characters, e.g., 
   Acked-by ''\(this helps differentiate the footer section from a multi-
   paragraph body''\). An exception is made for BREAKING CHANGE, which MAY
